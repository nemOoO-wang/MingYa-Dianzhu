...
-(void)updateBtnWithName:(NSString *)shopName andProjectId:(NSString *)projectId{
    ...
    // add button
    self.btn = [[UIButton alloc]initWithFrame:CGRectMake(0, 0, NMCalloutWidth, NMCalloutHeight)];//99\44
//    UIImage *img = [UIImage imageNamed:@"sec_shop"];
//    [self.btn setBackgroundImage:img forState:UIControlStateNormal];
    [self.btn setTitle:shopName forState:UIControlStateNormal];
    [self.btn.titleLabel setFont:[UIFont systemFontOfSize:NMCalloutFontSize]];
    [self.btn setTitleEdgeInsets:UIEdgeInsetsMake(-20, -10, 0, 0)];
    [self.btn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    CGPoint originCenter = self.btn.center;
    self.btn.center = CGPointMake(originCenter.x - CGRectGetWidth(self.btn.bounds)/6.f, originCenter.y - CGRectGetHeight(self.btn.bounds) + NMCalloutBottomOffset);
}
